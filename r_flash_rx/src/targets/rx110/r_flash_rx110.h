...
#define FLASH_MIN_PGM_SIZE_DF      (1)
#define FLASH_MIN_PGM_SIZE_CF      (4)
...
#define FLASH_DF_FULL_PGM_SIZE     (FLASH_DF_FULL_SIZE-FLASH_MIN_PGM_SIZE_DF)
...
#define FLASH_NUM_BLOCKS_CF (BSP_ROM_SIZE_BYTES) / (FLASH_CF_BLOCK_SIZE)
#define FLASH_CF_FULL_SIZE         (FLASH_NUM_BLOCKS_CF*FLASH_CF_BLOCK_SIZE)
#define FLASH_CF_LOWEST_VALID_BLOCK  (FLASH_CF_BLOCK_INVALID + FLASH_CF_BLOCK_SIZE)
#define FLASH_CF_LAST_VALID_ADDR     (FLASH_CF_LOWEST_VALID_BLOCK)

#define FLASH_CF_256KBOUNDARY      (FLASH_CF_LOWEST_VALID_BLOCK + 0x3FFFF)
...
typedef enum _flash_block_address
{
...
#if   BSP_CFG_MCU_PART_MEMORY_SIZE == 0x6   // rx110 'H' part 8KB ROM
    FLASH_CF_BLOCK_INVALID = 0xFFFFDC00,
#else
...
#if   BSP_CFG_MCU_PART_MEMORY_SIZE == 0x0   //  'J' parts 16KB ROM
    FLASH_CF_BLOCK_INVALID = 0xFFFFBC00,
#else
...
#if   BSP_CFG_MCU_PART_MEMORY_SIZE == 0x1   // '1' parts 32KB ROM
    FLASH_CF_BLOCK_INVALID = 0xFFFF7C00,
#else
...
#if   BSP_CFG_MCU_PART_MEMORY_SIZE == 0x3   //   '3' parts 64KB ROM
    FLASH_CF_BLOCK_INVALID = 0xFFFEFC00,
#else
...
#if   BSP_CFG_MCU_PART_MEMORY_SIZE == 0x4   //   '4' parts 96KB ROM
    FLASH_CF_BLOCK_INVALID = 0xFFFE7C00,
#else
    ...
    FLASH_CF_BLOCK_INVALID = 0xFFFDFC00,
#endif
#endif
#endif
#endif
...
/* Define the clock frequency supplied to the Flash. */
...
#define WAIT_MAX_ROM_WRITE \
        ((int32_t)(490 * (32.0/(FLASH_CLOCK_HZ/1000000)))*(BSP_ICLK_HZ/1000000))
...
#define WAIT_MAX_DF_WRITE \
        ((int32_t)(374 * (32.0/(FLASH_CLOCK_HZ/1000000)))*(BSP_ICLK_HZ/1000000))
...
#define WAIT_MAX_BLANK_CHECK \
        ((int32_t)(16 * (32.0/(FLASH_CLOCK_HZ/1000000)))*(BSP_ICLK_HZ/1000000))
...
#define WAIT_MAX_ERASE_CF_1K \
        ((int32_t)(214000 * (32.0/(FLASH_CLOCK_HZ/1000000)))*(BSP_ICLK_HZ/1000000))
...
#define WAIT_MAX_ERASE_DF_1K \
        ((int32_t)(228000 * (32.0/(FLASH_CLOCK_HZ/1000000)))*(BSP_ICLK_HZ/1000000))
/* Define the clock frequency supplied to the Flash circuit. On the RX111 it is the FCLK. */
...
//    #error "ERROR - Flash API - PCLK on RX62G must be between 8MHz and 50MHz!"
//#endif


// RX111 specific implementations
...
flash_err_t flash_pe_mode_enter();
