...
#pragma interrupt Interrupt_USBI0(vect=VECT(USB0, USBR0))
void Interrupt_USBI0(void)
{
	USBHALInterruptHandler();
}

//#pragma interrupt Interrupt_USBI1(vect=VECT(USB0, D1FIFO0))
...
USB_ERR USBHAL_Control_IN(uint16_t _NumBytes, const uint8_t* _Buffer)
{
	...
	else
	{
		...
		sprintf(tempstr, "USBHAL: CONTROL IN start, %u bytes.\r\n", _NumBytes);
						 ...
						 CB_DONE_OUT _CBDone)
{
	...
	else
	{
		...
		sprintf(tempstr, "USBHAL: CONTROL OUT start, %u bytes.\r\n", _NumBytes);
		DEBUG_MSG_MID((tempstr));
						...
						CB_DONE_OUT _CBDone)
{
	...
	else
	{
		...
		else
		{
			...
			sprintf(tempstr, "USBHAL: BULK OUT start, %lu bytes.\r\n", _NumBytes);
			DEBUG_MSG_MID((tempstr));
...
USB_ERR USBHAL_Bulk_IN(uint32_t _NumBytes, const uint8_t* _Buffer, CB_DONE _CBDone)
{
	...
	else
	{
		...
		else
		{
			...
			sprintf(tempstr, "USBHAL: BULK IN start, %lu bytes.\r\n", _NumBytes);
			DEBUG_MSG_MID((tempstr));
							 ...
							 const uint8_t* _Buffer, CB_DONE _CBDone)
{
	...
	else
	{
		...
		else
		{
			...
			sprintf(tempstr, "USBHAL: INT IN start, %lu bytes.\r\n", _NumBytes);
			DEBUG_MSG_MID((tempstr));
...
//	USBIO.D1FIFOSEL.BIT.MBW = 1;
	...
	if(0 != g_Bulk.m_OUT.m_BuffSize)
	{
			...
			(Count != BULK_OUT_PACKET_SIZE))
		{
			...
			sprintf(tempstr, "USBHAL: - ReadBulkOUTPacket Done, %lu Bytes\r\n", g_Bulk.m_OUT.m_DataBuff.NumBytes);
			DEBUG_MSG_MID( (tempstr));
...
{	
	...
	sprintf(tempstr, "USBHAL: INT IN - sending %lu, %lu remaining\r\n", Count, g_Int.m_IN.m_DataBuff.NumBytes);
	DEBUG_MSG_HIGH((tempstr));
...
static void WriteBulkINPacket(void)
{
	...
	sprintf(tempstr, "USBHAL: BULK IN - sending %lu, %lu remaining\r\n", Count, g_Bulk.m_IN.m_DataBuff.NumBytes);
	DEBUG_MSG_HIGH((tempstr));
...
//	USBIO.CFIFOSEL.BIT.MBW = 0;
	...
	sprintf(tempstr, "USBHAL: CONTROL IN - sending %u, %lu remaining\r\n", Count, g_Control.m_IN.m_DataBuff.NumBytes);
	DEBUG_MSG_HIGH((tempstr));
...
static void HW_Init_Module0(void)
{
	...
	MPC.P14PFS.BIT.PSEL = 0x11;
	MPC.P16PFS.BIT.PSEL = 0x11;	
    ...
    IPR(USB0, USBR0) = USB_HAL_INTERRUPT_PROIRITY;
    //IPR(USB0, D1FIFO0) = USB_HAL_INTERRUPT_PROIRITY;
