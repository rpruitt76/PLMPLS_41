...
*         : 31.03.2014 1.70     Added the ability for the user to define two 'warm start' callback functions which when
...
static void operating_frequency_set (void)
{
    ...
    /* Bottom byte of SCKCR register must be set to 0x11.
       b15 to b12 should be 1. */
    temp_clock |= 0x00001011;
...
static void clock_source_select (void)
{
    ...
    /* The delay period needed is to make sure that the HOCO has stabilized. According to Rev.1.00 of the RX63N's HW 
       ...
       in the LOCO. */
    R_BSP_SoftwareDelay(2400, BSP_DELAY_MICROSECS);
#else
    /* HOCO is not chosen. */
    /* Stop the HOCO. */
    ...
    R_BSP_SoftwareDelay(2600, BSP_DELAY_MILLISECS);
#else
