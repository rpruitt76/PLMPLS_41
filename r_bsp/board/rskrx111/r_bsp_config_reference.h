...
*         : 16.11.2012 1.10    Added BSP_CFG_USER_LOCKING_ENABLED and BSP_CFG_USER_LOCKING_TYPE macros.
*         : 19.11.2012 1.20    Updated code to use 'BSP_' and 'BSP_CFG_' prefix for macros.
*         : 17.01.2013 1.30    Moved BSP_CFG_IO_LIB_ENABLE macro since it is now used in conjunction with 
...
*         : 06.05.2013 1.40    Removed BSP_CFG_USER_APP_ENDIAN & BSP_CFG_USER_BOOT_ENDIAN macros. Endian is now detected
*                              using compiler macros. Added BSP_CFG_USE_CGC_MODULE macro so that users can use r_cgc_rx 
*                              module for clock management. Added BSP_CFG_MCU_VCC_MV macro for FIT modules that need to
*                              know Vcc.
*         : 25.06.2013 1.50    Updated product part number section based on v1.00 of RX111 HW manual. Added
*                              BSP_CFG_USER_STACK_ENABLE macro to enable/disable use of user stack. Removed
*                              macros defining exception callbacks, undefined interrupt callback, and bus error 
*                              callback. Callbacks are now handled through mcu_interrupts.c. Added
*                              BSP_CFG_BOARD_REVISION macro for choosing board revision.
...
/* The r_bsp can be configured to use the r_cgc_rx111 module for clock management. The r_bsp does come with code
   to setup the clocks but it provides no management capabilities and has no ability to change clock settings. If
   the user wishes to change the clocks after reset then they should use the r_cgc_rx111 module.
   0 = Use built-in clock code
   1 = Use r_cgc_rx111 module for clock management
*/
...
/* There are multiple versions of the RSKRX111. Examples of things that can change with different boards include LED
 * pins, switch pins, and which pin potentiometer is connected to. Choose which board is currently being used below.
 * 0 = 1st Prototype Board (has DB-9 connector, should have 'D011672_06_V01' on back)
 * 1 = Mass Production Board (has 2 PMOD connectors, virtual comm port w/RL78, should have 'D011672_06_V02' on back
 */
