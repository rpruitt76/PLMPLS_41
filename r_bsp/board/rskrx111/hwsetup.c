...
*         : 14.12.2012 1.10     Added port support for RSPI on header J3
*         : 07.05.2013 1.20     Added a call to bsp_non_existent_port_init() which initializes non-bonded out GPIO pins.
*         : 19.06.2013 1.30     Removed RSPI pin setup since this caused excess current draw in low power modes. Also
*                               replaced manual calls to unlock MPC registers with calls to r_bsp API functions.
*                               Moved call to bsp_non_existent_port_init() in hardware_setup() to last so that user
*                               does not have to worry about writing 'missing' bits in PDR registers. Added support
*                               for MP RSKRX111 which has potentiometer on P4_0.
