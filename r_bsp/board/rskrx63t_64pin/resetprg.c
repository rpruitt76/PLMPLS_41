...
*         : 11.10.2012 1.00     First port to RX63T
*         : 09.10.2012 1.10     Added support for all clock options specified by BSP_CFG_CLOCK_SOURCE in r_bsp_config.h.
...
*         : 26.06.2013 1.30     Removed auto-enabling of NMI pin interrupt handling. Fixed a couple of typos. Fixed
...
*         : 25.11.2013 1.31     LOCO is now turned off when it is not chosen as system clock.
*         : 31.03.2014 1.40     Added the ability for the user to define two 'warm start' callback functions which when
...
*                   Default sizes are USP=1K and ISP=512B.
...
static void operating_frequency_set (void)
{
    ...
    temp_clock |= 0x00010010;  // Always set b:4-7 and b-16-19 same as PCKB    
#elif BSP_CFG_PCKB_DIV == 4
    temp_clock |= 0x00000200;
    temp_clock |= 0x00020020;  // Always set b:4-7 and b-16-19 same as PCKB   
#elif BSP_CFG_PCKB_DIV == 8
    temp_clock |= 0x00000300;
    temp_clock |= 0x00030030;  // Always set b:4-7 and b-16-19 same as PCKB    
#elif BSP_CFG_PCKB_DIV == 16
    temp_clock |= 0x00000400;
    temp_clock |= 0x00040040;  // Always set b:4-7 and b-16-19 same as PCKB    
#elif BSP_CFG_PCKB_DIV == 32
    temp_clock |= 0x00000500;
    temp_clock |= 0x00050050;  // Always set b:4-7 and b-16-19 same as PCKB    
#elif BSP_CFG_PCKB_DIV == 64
    temp_clock |= 0x00000600;
    temp_clock |= 0x00060060;  // Always set b:4-7 and b-16-19 same as PCKB    
#else
...
static void clock_source_select (void)
{
    ...
    /* PLL wait is 1,048,576 cycles * 192 MHz (16 MHz * 12) = 5.46 ms*/
